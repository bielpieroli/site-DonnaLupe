...
function About() {
  ...
  const fallbackEssenceCards = [
    ['+10 anos', 'Levando sabor, carinho e qualidade para nossos clientes.'],
    ['Ingredientes reais', 'Trabalhamos com produtos selecionados e receitas artesanais.'],
    ['Atendimento especial', 'Queremos que cada cliente tenha uma experiência incrível.'],
    ['Receitas exclusivas', 'Sabores únicos para agradar todos os gostos.'],
  ]
  const fallbackValueCards = [
    ['💛', 'Feito com amor', 'Cada cookie é preparado com cuidado, atenção e muito carinho.'],
    ['🍪', 'Qualidade em cada detalhe', 'Utilizamos ingredientes selecionados para garantir sabor e frescor.'],
    ['✨', 'Experiência inesquecível', 'Queremos transformar cada compra em um momento especial.'],
  ]
    ...
    <main className="bg-bg text-[#3d0d12]">
        ...
        <div className="grid h-full w-full grid-cols-1 items-center gap-10 px-4 sm:px-6 lg:grid-cols-2 max-w-7xl justify-center mx-auto">
          <div className="text-primary-contrast">
            ...
            <h1 className="w-full font-display text-5xl lg:text-6xl xl:text-7xl font-extrabold leading-tight">
              {hero?.title || 'Feitos com amor desde a primeira fornada'}
            </h1>

            <p className="mt-6 w-full text-base sm:text-lg md:text-xl leading-7 md:leading-8 text-justify text-white/90">
              {hero?.description || 'Mais do que cookies, criamos momentos especiais. Cada receita é feita com ingredientes selecionados, carinho e aquele gostinho de casa que transforma qualquer dia.'}
            </p>
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          <div>
            ...
            <h2 className="mt-3 font-display text-5xl font-extrabold leading-tight">
              {essence?.title || 'Uma marca feita para adoçar momentos'}
            </h2>

            {(essenceParagraphs.length ? essenceParagraphs : [
              'Somos uma empresa dedicada a oferecer os melhores produtos e serviços para nossos clientes. Com anos de experiência no mercado, buscamos garantir qualidade, sabor e uma experiência memorável em cada pedido.',
      ...
      <section className="mx-auto max-w-6xl px-6 py-24 text-center bg-[radial-gradient(circle_at_center,rgba(215,38,77,0.06),transparent_60%)]">
        ...
        <p className="mx-auto mt-8 max-w-3xl text-xl leading-8 text-[#5a4a49]">
          {cta?.description || 'Explore nosso cardápio e descubra os cookies que conquistaram nossos clientes.'}
        </p>
